...
async function mintSignedEnvelope(
  ...
  // length-prefixed, so no two field splits can collide. `:v2` also covers the
  // policy EPOCH and the AUTHORITY (DR-0063 §5), which is why the hosted path
  // can read the epoch from the signature instead of taking it as an argument.
  let preimage = "whipplescript-governance-envelope:v2;";
  for (const item of [
    envelopeHash,
    signer,
    "p256-sha256",
...
async function bootstrapSession(
  ...
  // Minted rather than a hand-signed constant: a `:v2` attestation covers the
  // epoch and the authority, so a static blob could only be regenerated by
  // whoever held its key.
  const minted = envelope ?? (await mintSignedEnvelope(
    withTools ? ["workspace.read", "workspace.write", "command.run"] : [],
  ));
  const bootstrap = await stub.fetch(
    "https://session.test/public/session/bootstrap",
    {
      ...
      body: JSON.stringify({
        ...
        host_policy: {
          epoch: 1,
          signed_envelope: minted.text,
          expected_signer: minted.signer,
          signer_public_key_hex: minted.publicKeyHex,
...
describe("real WorkflowInstance hibernation", () => {
  ...
  it("rehydrates durable state and the browser socket attachment after object eviction", async () => {
    ...
    const minted = await mintSignedEnvelope();
