...
mod tests {
    ...
    /// Shared, so a test can hold the bucket it handed to the store and ask
    /// afterwards what is actually in it.
    impl crate::ObjectStore for std::rc::Rc<MemObjects> {
    ...
    fn a_registered_handle_is_live_unreadable_here_and_erasable() {
        ...
        assert_eq!(
            ...
            vec![crate::do_branches::PendingExternalDelete {
                id: id.to_owned(),
                storage_key: id.to_owned(),
            }],
            "and the bytes are queued for the plane to collect, under the key a \
             push stored them with: its own id"
        );
    ...
    /// Ingested bytes are stored under a key minted before the digest existed,
    /// and everything downstream has to follow them there.
    ///
    /// The collection queue is where this bites hardest. A collector that
    /// assumed the id was the key would delete an object that was never
    /// written, report success, and leave the real bytes in the bucket — an
    /// erasure that says it happened and did not, which is the one outcome
    /// DR-0066 §5 spends its whole argument on.
    #[test]
    fn ingested_bytes_are_collected_under_the_key_they_were_stored_with() {
        use whipplescript_store::content::{ContentBlobs, EraseOutcome};
        let blobs =
            crate::do_branches::DoContentBlobs::new(RusqliteDoSql::in_memory()).expect("open");
        let id = "89504e470d0a1a0a0000000d0000000d";
        let staged = "s-0123456789abcdef0123456789abcdef";

        let in_force = blobs
            .register_external_at(id, 4096, Some(staged))
            .expect("register an ingested handle");
        assert_eq!(in_force, staged, "the key it offered is the key it got");
        assert_eq!(
            blobs.external_storage_key(id).expect("resolve"),
            staged,
        ...
        assert_eq!(
            blobs.external_storage_key(id).expect("resolve"),
            id,
            "and the mapping goes once nothing is owed — it exists to find bytes"
        );
    }

    /// A host that can drive its own store deletes the object that is there.
    ///
    /// The queue path and this one are different code, and only one of them
    /// was ever exercised by a bucket. Resolving the key in one and not the
    /// other would leave ingested bytes behind on exactly the hosts that can
    /// reach them.
    #[test]
    fn a_synchronous_store_erases_the_object_the_key_names() {
        use whipplescript_store::content::{ContentBlobs, EraseOutcome};
        let objects = std::rc::Rc::new(MemObjects::default());
        let blobs = crate::do_branches::DoContentBlobs::with_external_bytes(
            RusqliteDoSql::in_memory(),
            Box::new(std::rc::Rc::clone(&objects)),
        )
        .expect("open");
        let id = "89504e470d0a1a0a0000000d0000000d";
        let staged = "s-0123456789abcdef0123456789abcdef";
        // The plane put the bytes there; this records where.
        crate::ObjectStore::put(&*objects, staged, b"generated bytes").expect("stage");
        blobs
            .register_external_at(id, 15, Some(staged))
            .expect("register");

        assert_eq!(
            blobs.get(id).expect("read"),
            Some(b"generated bytes".to_vec()),
            "a reader asking by id is answered from the staged key"
        );
        assert!(matches!(
            blobs.erase(id, "2026-09-15T00:00:00Z").expect("erase"),
            EraseOutcome::Erased { byte_len: 15 }
        ));
        assert!(
            !crate::ObjectStore::exists(&*objects, staged),
            "and erasure reaches the object rather than the id it is filed under"
        );
    }

    /// The first registration owns the id, and the second is told so.
    ///
    /// Two ingests of identical bytes mint two keys and write two objects, and
    /// only one can be what the id means. Letting the later one win would move
    /// bytes out from under a reader that already resolved the id — the
    /// substitution content addressing exists to prevent — so it loses, and
    /// gets back the winner's key so it knows to collect what it orphaned.
    #[test]
    fn the_first_registration_owns_the_id_and_a_later_one_learns_it_lost() {
        let blobs =
            crate::do_branches::DoContentBlobs::new(RusqliteDoSql::in_memory()).expect("open");
        let id = "89504e470d0a1a0a0000000d0000000d";
        let first = "s-11111111111111111111111111111111";
        let second = "s-22222222222222222222222222222222";

        assert_eq!(
            blobs
                .register_external_at(id, 4096, Some(first))
                .expect("first"),
            first
        );
        assert_eq!(
            blobs
                .register_external_at(id, 4096, Some(second))
                .expect("second"),
            first,
            "the id keeps the bytes it already meant"
        );
        assert_eq!(blobs.external_storage_key(id).expect("resolve"), first);
    }

    /// A key the plane did not mint is a caller-chosen name in the bucket.
    ///
    /// Keying on the hash is what kept arbitrary names out of that namespace.
    /// A mapping table gives them a second door, and it has to be as narrow as
    /// the first — otherwise an id can be pointed at bytes it does not
    /// describe, which is the whole property this tier sells.
    #[test]
    fn a_staged_key_must_be_one_the_plane_minted() {
        let blobs =
            crate::do_branches::DoContentBlobs::new(RusqliteDoSql::in_memory()).expect("open");
        let id = "89504e470d0a1a0a0000000d0000000d";
        for bad in [
            ...
            "s-",
            "../../etc/passwd",
            "s-not-hex-not-hex-not-hex-not-hex",
            "s-0123456789ABCDEF0123456789ABCDEF",
            &format!("s-{}", "0".repeat(31)),
            &format!("s-{}", "0".repeat(33)),
            // A bare content id: legal as a key, but only for a push, which
            // says so by naming no key at all.
            id,
        ] {
            assert!(
                blobs.register_external_at(id, 1, Some(bad)).is_err(),
                "{bad:?} must be refused"
            );
        }
    }

    /// Rows written before ingest existed name no key, and mean their id.
    ///
    /// This is what makes the mapping additive: the table can be empty for a
    /// whole live store and every answer in it is still right.
    #[test]
    fn a_handle_with_no_mapping_resolves_to_its_own_id() {
        let blobs =
            crate::do_branches::DoContentBlobs::new(RusqliteDoSql::in_memory()).expect("open");
        let id = "89504e470d0a1a0a0000000d0000000d";
        blobs.register_external(id, 4096).expect("register a push");
        assert_eq!(blobs.external_storage_key(id).expect("resolve"), id);
    }
