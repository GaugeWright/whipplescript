...
fn redirected_request_does_not_bypass_the_host_allow_list() {
    ...
    let server = std::thread::spawn(move || {
        ...
        let count = stream.read(&mut raw).expect("read allowed request");
        let seen = String::from_utf8_lossy(&raw[..count]);
        assert!(seen.contains("X-Api-Key: synthetic-secret"), "{seen}");
        let reply = format!(
