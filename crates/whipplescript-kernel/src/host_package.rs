...
pub fn workspace_tool_specs(writable: bool) -> Vec<ToolSpec> {
    workspace_tool_specs_with_capabilities(writable, false)
}

pub fn workspace_tool_specs_with_command(writable: bool, command_execution: bool) -> Vec<ToolSpec> {
    workspace_tool_specs_with_capabilities(writable, command_execution)
}
...
pub fn workspace_tool_specs_with_capabilities(
    ...
    workspace_tool_specs_from_registry(true, writable, command_execution)
}
...
"#;
    ...
    #[test]
    fn ability_ceiling_narrows_tools_without_rewriting_source() {
        let source = r#"
workflow Chat {
  agent assistant {
    provider owned
...
"#;
    ...
    #[test]
    fn ability_ceiling_must_be_a_safe_subset() {
        let source = r#"
workflow Chat {
  agent assistant {
    provider owned
...
"#;
        let manifest = json!({
            ...
            "system_prompt": "persona.md",
            "capabilities": ["workspace.read"],
            "agent_abilities": ["workspace.write"],
            "max_steps": 12
        });
        let error =
            AuthoredAgentPackage::from_documents(manifest.to_string(), source, "Be useful.")
                .expect_err("ability widening must fail");
        assert!(error.contains("absent from the package capability registry"));
    }
}
