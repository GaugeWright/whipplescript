...
fn primitive_schema(primitive: &IrPrimitiveType) -> Value {
    match primitive {
        ...
        IrPrimitiveType::Secret(_) => json!(false),
    }
}
...
/// Why this model cannot serve a generation here, or `None` if it can.
///
/// **The image models that return base64 are refused.** `gpt-image-1` does not
/// support `response_format` at all and always answers with `b64_json`
/// ([OpenAI images reference]), and base64 is the one answer this runtime cannot
/// take: the bytes would land in the isolate, which DR-0033 Decision 4 forbids
/// — "the isolate touches only handle + metadata, never the bytes" — and which
/// the durable object cannot do at any size, because `SqlValue` carries Null,
/// Int and Text and no blob.
///
/// A URL, by contrast, is exactly what DR-0119's ingest was built to take. So
/// the constraint is not a preference between models; it is which of them this
/// storage tier can hold the output of. Refusing by name at request time says
/// that once, instead of failing after the spend with a base64 payload nobody
/// can store.
///
/// [OpenAI images reference]: https://platform.openai.com/docs/api-reference/images
pub fn media_model_refusal(model: &str) -> Option<String> {
    let base64_only = model.starts_with("gpt-image");
    base64_only.then(|| {
        format!(
            "`{model}` returns base64 image data and does not support \
             `response_format`, and this runtime stores generated bytes by \
             reference: the isolate never holds them (DR-0033 Decision 4). Use a \
             model that answers with a URL — `dall-e-3` — so the object tier can \
             ingest it."
        )
    })
}

/// The provider request for a `prompt "…" -> image` (DR-0120).
///
/// `POST {base_url}/v1/images/generations`, which is a different endpoint and a
/// different body from every other request in this module: a generation is not
/// a structured-output call, so there is no schema, no messages array and no
/// tool. `response_format: "url"` is the load-bearing field — see
/// [`media_model_refusal`] for why base64 is not an option here — and `n: 1`
/// because one effect binds one artifact.
///
/// The returned URL is short-lived (60 minutes), which is why the artifact is
...
/// The URL a generation answered with, or why the answer is not one.
///
/// `{"data": [{"url": …}]}`. A `b64_json` answer is a REFUSAL rather than a
/// decode: a model that ignored `response_format` has handed back bytes this
/// runtime will not put in the isolate, and silently storing them would be the
/// substitution the storage tier exists to prevent.
pub fn parse_media_response(response: &HttpResponse) -> Result<String, String> {
    if response.status >= 400 {
        let message = response
            ...
            .unwrap_or("the image provider refused the request");
        return Err(format!(
    ...
    let first = response
        ...
        .ok_or("the image provider returned no image")?;
    if let Some(url) = first.get("url").and_then(Value::as_str) {
        return Ok(url.to_owned());
    }
    if first.get("b64_json").is_some() {
        return Err(
            "the image provider answered with base64 despite `response_format: url`; this \
             runtime stores generated bytes by reference and the isolate never holds them"
                .to_owned(),
        );
    }
    Err("the image provider's answer carried neither a url nor image data".to_owned())
}
...
mod tests {
    ...
    fn image_call<'a>(model: &'a str, schema: &'a Value) -> CoerceCall<'a> {
        CoerceCall {
            provider: CoerceProvider::OpenAi,
            base_url: "https://api.openai.com/",
            api_key: "sk-test",
            model,
            prompt: "Draw a poster for autumn harvest",
            media: None,
    ...
    /// A generation is a different endpoint and a different body from every
    /// other request this module builds — no schema, no messages, no tool.
    #[test]
    fn a_generation_asks_the_images_endpoint_for_a_url() {
        let schema = json!({"type": "string"});
        let request = build_media_request(&image_call("dall-e-3", &schema));

        assert_eq!(request.url, "https://api.openai.com/v1/images/generations");
        assert_eq!(request.body["model"], "dall-e-3");
        assert_eq!(request.body["prompt"], "Draw a poster for autumn harvest");
        assert_eq!(request.body["n"], 1);
        // The load-bearing field: base64 is the one answer this runtime cannot
        // take, because the bytes would land in the isolate.
        assert_eq!(request.body["response_format"], "url");
        // A structured-output request's furniture must not appear.
        assert!(request.body.get("messages").is_none());
        assert!(request.body.get("response_format").unwrap().is_string());
    ...
    /// `gpt-image-1` does not support `response_format` and always answers with
    /// base64, which this runtime will not put in the isolate (DR-0033
    /// Decision 4) and the durable object could not hold at any size.
    ///
    /// Refusing by NAME at request time says so once. The alternative is failing
    /// after the spend, holding a payload nobody can store.
    #[test]
    fn a_base64_only_model_is_refused_before_the_request_is_sent() {
        let refusal = media_model_refusal("gpt-image-1").expect("returns base64 only");
        assert!(
            refusal.contains("dall-e-3"),
            "the refusal names a model that would work: {refusal}"
        );
        assert!(media_model_refusal("gpt-image-1-mini").is_some());
        // The URL-answering models are not refused.
        assert_eq!(media_model_refusal("dall-e-3"), None);
        assert_eq!(media_model_refusal("dall-e-2"), None);
    }

    /// The answer is a URL, and anything else is said rather than guessed at.
    #[test]
    fn a_generation_answer_yields_a_url_or_says_why_not() {
        let ok = HttpResponse {
            status: 200,
            body: json!({"created": 1, "data": [{"url": "https://cdn.invalid/a.png"}]}),
        };
        assert_eq!(
            parse_media_response(&ok).expect("a url"),
            "https://cdn.invalid/a.png"
        );

        // A model that ignored `response_format` hands back bytes. That is a
        // refusal, not a decode: storing them would be exactly the substitution
        // the storage tier exists to prevent.
        let base64 = HttpResponse {
            status: 200,
            body: json!({"data": [{"b64_json": "aGVsbG8="}]}),
        };
        let refused = parse_media_response(&base64).expect_err("base64 is refused");
        assert!(
            refused.contains("base64") && refused.contains("isolate"),
            "the refusal says what it will not do and why: {refused}"
        );

        // A provider error carries its own message through.
        let failed = HttpResponse {
            status: 400,
            body: json!({"error": {"message": "content policy"}}),
        };
        let error = parse_media_response(&failed).expect_err("400 is a failure");
        assert!(error.contains("content policy"), "{error}");
        ...
        assert!(
            parse_media_response(&empty).is_err(),
            "no image is a failure"
        );

        // An entry that is NEITHER — a revised prompt echoed back with no image,
        // which this endpoint does return alongside a result. On its own it is
        // not an answer, and reporting success would bind a media value with
        // nothing behind it. Distinct from the empty-array case above, which
        // never reaches this arm.
        let neither = HttpResponse {
            status: 200,
            body: json!({"data": [{"revised_prompt": "a poster, autumnal"}]}),
        };
        let unanswered = parse_media_response(&neither)
            .expect_err("an entry carrying no image is not an answer");
        assert!(
            unanswered.contains("neither a url nor image data"),
            "and it says what was missing: {unanswered}"
        );
    }
