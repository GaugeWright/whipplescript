//! Tracker codecs extend the immutable recording bundle. Reports cross the
//! version boundary as JSON so its private harness types need no modification.
#[path = "host_action_contract_v3.rs"]
mod recording;
...
use whipplescript_kernel::{
    host_protocol::tracker_recovery::RecoverTrackerResult,
...
use whipplescript_store::{
    ...
    tracker_filing::{TrackerFiling, TrackerFilingReceipt},
};
...
pub fn contract_reports() -> Vec<Value> {
    let vectors: Vectors = serde_json::from_str(include_str!(
        "../../../spec/host-action-contract-fixtures-v4.json"
    ))
    .expect("pinned tracker vectors");
    assert_eq!(
        vectors.schema,
        "whipplescript.host_action_contract_fixtures.v4"
    );
    assert_eq!(
        vectors.contract_revision,
        "whipplescript-host-action/v4.0.0"
    );
    ...
    let bases: BTreeMap<_, _> = vectors
        ...
        .collect();
    let mut reports: Vec<Value> = recording::contract_reports()
        .into_iter()
    ...
    for case in &vectors.cases {
        ...
        let observation = match case.message_type.as_str() {
            "TrackerBinding" => observe::<TrackerBinding>(&value),
            "TrackerClosureBinding" => observe::<TrackerClosureBinding>(&value),
            "FilingDispatch" => observe::<FilingDispatch>(&value),
            "ClosureDispatch" => observe::<ClosureDispatch>(&value),
            "TrackerFilingReceipt" => observe::<TrackerFilingReceipt>(&value),
            "TrackerClosureReceipt" => observe::<TrackerClosureReceipt>(&value),
            "TrackerFiling" => {
                let mut report = observe::<TrackerFiling>(&value);
                if let Ok(request) = serde_json::from_value::<TrackerFiling>(value.clone()) {
                    let fingerprint = request.fingerprint().ok();
                    report["syntax_valid"] = json!(fingerprint.is_some());
                    report["fingerprint"] = json!(fingerprint);
                }
                report
            }
            "TrackerClosure" => {
                let mut report = observe::<TrackerClosure>(&value);
                if let Ok(request) = serde_json::from_value::<TrackerClosure>(value.clone()) {
                    let fingerprint = request.fingerprint().ok();
                    report["syntax_valid"] = json!(fingerprint.is_some());
                    report["fingerprint"] = json!(fingerprint);
                }
            ...
            "RecoverTrackerResult" => {
                let mut report = observe::<RecoverTrackerResult>(&value);
                if let Ok(request) = serde_json::from_value::<RecoverTrackerResult>(value.clone()) {
                    let bytes = request.signing_bytes().ok();
                    report["syntax_valid"] = json!(bytes.is_some());
                    report["signing_sha256"] = json!(bytes.map(|bytes| {
                        Sha256::digest(bytes)
                            .iter()
                            .map(|b| format!("{b:02x}"))
                            .collect::<String>()
            ...
            other => panic!("unregistered tracker contract type {other}"),
        };
        assert_eq!(
...
fn main() {
    println!(
        "{}",
        serde_json::to_string(&contract_reports()).expect("tracker reports")
    );
}
