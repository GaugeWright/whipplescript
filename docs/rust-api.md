...
| `TurnExecution::usage` / `TurnUsageObservation` | The narrow typed projection of the usage of the settled turn: the summed counts of the tokens of the turn for a meter of a product, and `last_input_tokens` — the size of the prompt of the final main call, the reading of the fullness of the context window that the compaction trigger of the runtime itself consumes. The `usage_ref` of the receipt remains the authoritative pointer to the evidence. |
