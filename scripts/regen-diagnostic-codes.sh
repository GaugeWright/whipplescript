...
ARTIFACT_LEDGER="spec/diagnostic-codes-artifact.txt"
...
# The SECOND family: report-validator codes minted with `format!` under
# `construct_graph.` / `lowered_ir.`, registered by SHAPE (DR-0107). A
# placeholder stays a placeholder (`{owner_kind}` -> `<owner_kind>`), and
# anything after a `:` is a LOCATOR -- an object id or value -- not the code.
ARTIFACT_LITERAL = re.compile(r'"((?:construct_graph|lowered_ir)\.[^"]*)"')
def artifact_shape(literal):
    code = literal.split(":", 1)[0]
    return re.sub(r"\{([A-Za-z_][A-Za-z0-9_]*)\}", r"<\1>", code)
# A literal that is NOTHING but the family and one placeholder --
# `construct_graph.{ref_spec}` -- is a construct REF being assembled, the
# locator a diagnostic points at, not a code: a code names a refusal in at
# least one literal segment. The regex cannot tell the two apart, so the
# shape does.
ARTIFACT_REF_SHAPE = re.compile(r"^(?:construct_graph|lowered_ir)\.<[A-Za-z_][A-Za-z0-9_]*>$")
def artifact_shapes(text):
    return {
        shape
        for shape in (artifact_shape(literal) for literal in ARTIFACT_LITERAL.findall(text))
        if not ARTIFACT_REF_SHAPE.match(shape)
    }
def test_sources():
    for crate in sorted(os.listdir("crates")):
        src = os.path.join("crates", crate, "src")
        if not os.path.isdir(src):
            continue
        for directory, _, files_here in os.walk(src):
            for name in sorted(files_here):
                if not name.endswith(".rs"):
                    continue
                path = os.path.join(directory, name)
                if "_tests" in path or "/tests/" in path or name[:-3].endswith("_tests"):
                    yield path
artifact, artifact_named_by_a_test = set(), set()
files = 0
for path in shipped_sources():
    ...
    artifact |= artifact_shapes(text)
for path in test_sources():
    with open(path, encoding="utf-8") as handle:
        artifact_named_by_a_test |= artifact_shapes(handle.read())
if files <= 100:
    sys.exit(
...
with open(os.path.join(work, "artifact-codes"), "w", encoding="utf-8") as handle:
    for code in sorted(artifact):
        mark = "COVERED" if code in artifact_named_by_a_test else "PROVISIONAL"
        handle.write(f"{code}\t{mark}\n")
PYTHON
...
# ---------------------------------------------------------------------------
# 4. The artifact ledger: the report validators' codes, registered by SHAPE.
# ---------------------------------------------------------------------------
{
  echo "# The ARTIFACT diagnostic code register — the codes \`whip verify\` and the"
  echo "# report validators refuse a construct_graph or lowered_ir artifact with."
  echo "# GENERATED by scripts/regen-diagnostic-codes.sh from the string literals"
  echo "# under construct_graph. and lowered_ir. in the shipped sources; do not"
  echo "# hand-edit."
  echo "#"
  echo "# Registered by SHAPE (DR-0107). A <placeholder> is a slot the artifact fills"
  echo "# -- an owner kind, a field name -- and stays a placeholder here, so the"
  echo "# ledger is finite. Anything after a ':' in an emitted spelling is a LOCATOR"
  echo "# naming which object, and is not part of the code."
  echo "#"
  echo "# Second column: COVERED when a test source names the shape, PROVISIONAL"
  echo "# when none does. The refusal fixtures under the CLI's main_tests are the"
  echo "# corpus; a COVERED shape is frozen under the append-only guarantee, a"
  echo "# PROVISIONAL one may still be corrected."
  cat "$work/artifact-codes"
...
compare_or_write "$work/artifact-ledger" "$ARTIFACT_LEDGER" \
  "an artifact code was allocated or stopped being emitted" || status=1
